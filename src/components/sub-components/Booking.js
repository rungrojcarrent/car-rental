...
export default function Booking() {
  ...
  function checkField(e) {
    ...
    if (carPicked) {
        // Optimization: Use .find() instead of .filter()[0] to avoid creating an intermediate array
        // and stop iterating once a match is found.
        const foundCar = cars.find(item => item.name === carPicked);
        if (foundCar) setCarImg(foundCar.image);
    }
